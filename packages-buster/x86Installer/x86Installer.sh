...
exit_error() {
  ...
  if isMounted /tmp/diesis; then
    /bin/umount /tmp/diesis
  fi

  [ -d "/tmp/boot" ] && /bin/rm -r /tmp/boot
  [ -d "/tmp/diesis" ] && /bin/rm -r /tmp/diesis
...
/sbin/mkfs -F -t ext4 -L diesis ${7} > /dev/null 2>&1
/bin/echo "35" > /tmp/install_progress
/sbin/mkfs -F -t ext4 -L diesis_data ${8} > /dev/null 2>&1
...
/bin/mkdir /tmp/boot
/bin/mkdir /tmp/diesis
/bin/mount $6 /tmp/boot
/bin/mount $7 /tmp/diesis
...
/bin/rm -r /tmp/diesis/lost+found
/bin/umount /tmp/boot
/bin/umount /tmp/diesis
/bin/rm -r /tmp/boot
/bin/rm -r /tmp/diesis
